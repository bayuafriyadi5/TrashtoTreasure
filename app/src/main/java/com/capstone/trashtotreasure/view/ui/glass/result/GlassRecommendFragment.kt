...
@AndroidEntryPoint
class GlassRecommendFragment : Fragment() {

    private var _binding: FragmentGlassRecommendBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentGlassRecommendBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        viewModel.snapGlass(file).observe(viewLifecycleOwner) { result ->
            when (result) {
                ...
                is Result.Success -> {
                    ...
                    val recommendationItems = data?.map { GlassRecommendationItem(it.orEmpty()) } ?: emptyList()
                    binding.rvRecommend.apply {
                        adapter = GlassRecommendAdapter(recommendationItems)
