...
import java.io.File
@AndroidEntryPoint
class MetalResultActivity : AppCompatActivity() {

    private val binding: ActivityMetalResultBinding by lazy {
        ActivityMetalResultBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val sectionsPagerAdapter = MetalPagerAdapter(this)
        binding.viewPager.adapter = sectionsPagerAdapter

        TabLayoutMediator(binding.tabs, binding.viewPager) { tab, position ->
            tab.text = resources.getString(MetalResultActivity.TAB_TITLES[position])
        }.attach()
    ...
    private fun getResults(file: File) {
        viewModel.snapMetal(file).observe(this) { result ->
            when (result) {
                is Result.Loading -> {
                    binding.tabs.visibility = View.GONE
                    binding.errorAnimation.visibility = View.GONE
