...
import com.capstone.trashtotreasure.databinding.FragmentGlassDescBinding
import com.capstone.trashtotreasure.databinding.FragmentMetalDescBinding
...
@AndroidEntryPoint
class GlassDescFragment : Fragment() {

    private var _binding: FragmentGlassDescBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentGlassDescBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun bindResult() {
        val activity = requireActivity() as GlassResultActivity
    ...
    private fun getResults(file: File) {
        viewModel.snapGlass(file).observe(viewLifecycleOwner) { result ->
            when (result) {
                is Result.Loading -> {
                    binding.loading.visibility = View.VISIBLE
                    binding.tvTitle.visibility = View.GONE
