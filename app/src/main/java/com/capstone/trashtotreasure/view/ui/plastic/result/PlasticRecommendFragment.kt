...
import com.capstone.trashtotreasure.view.ui.adapter.PlasticRecommendAdapter
import com.capstone.trashtotreasure.view.ui.adapter.PlasticRecommendationItem
import dagger.hilt.android.AndroidEntryPoint
import java.io.File

@AndroidEntryPoint
class PlasticRecommendFragment : Fragment() {

    private var _binding: FragmentPlasticRecommendBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPlasticRecommendBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        viewModel.snapPlastic(file).observe(viewLifecycleOwner) { result ->
            when (result) {
                ...
                is Result.Success -> {
                    ...
                    val recommendationItems = data?.map { PlasticRecommendationItem(it.orEmpty()) } ?: emptyList()
                    binding.rvRecommend.apply {
                        adapter = PlasticRecommendAdapter(recommendationItems)
